...
{ 
    ...
    public class VertExmotionEditor : Editor {
        ...
        List<int> m_vtxBrush = new List<int>();
        ...
        int[] m_unityVersion = null;

        public static Shader GetEditorShader()
        {
            Shader shader = Shader.Find("Hidden/HDRP/VertExmotion_editor");
            if (shader == null)
        ...
        {
            Shader shader = Shader.Find("Hidden/HDRP/VertExmotion_hidden");
            if (shader == null)
        ...
        {
            Shader shader = Shader.Find("Hidden/HDRP/VertExmotion_wireframe");
            if (shader == null)
    ...
    */
    ...
    /*
            ...
            if ( (m_mode == eMode.PAINT || m_mode == eMode.SENSORS) && !Application.isPlaying )
		    {
			    ...
			    if( !m_modeTrigger )
			    {
                    ...
                    for (int i = 0; i < sharedmaterials.Length; ++i)
                    {
                        ...
                        //--------------------------------------------
                        Shader shaderEditor = GetEditorShader();
        ...
        */
        ...
        void CreateCollider( VertExmotionBase vtm )
        {
        ...
        RaycastHit[] m_hits = new RaycastHit[20];
        ...
        void PaintVertice( VertExmotionBase vtm, Camera svCam, Vector3 mp, Vector2 mp2d )
	    {
            ...
            constUnit = HandleUtility.GetHandleSize(svCam.transform.position + svCam.transform.forward) * 10f;
            ...
            float paintRange = (svCam.WorldToScreenPoint(svCam.transform.position+ svCam.transform.forward) - svCam.WorldToScreenPoint(svCam.transform.position + svCam.transform.forward + svCam.transform.right * m_brushSize)).magnitude;
            //Debug.Log("paintRange : " + paintRange);
            

            //raycast
            int maxRc = (int) (Mathf.Lerp(100,1000, m_drawAccuracy) * m_drawRadius * 100f / 25f);
            for (int rc = 0; rc < maxRc; ++rc)
            {

                //Vector2 offset = new Vector2(((float)rc - 5f) / 10f, 0);
                float p = (float)rc / (float)maxRc;
                float a = Mathf.PI * 2f * p * 5f + System.DateTime.Now.Millisecond;
                Vector2 offset = new Vector2(Mathf.Cos(a), Mathf.Sin(a)) * p ;

                offset = Random.insideUnitCircle * p;


#if UNITY_2019_1_OR_NEWER
                PhysicsScene pscn = PhysicsSceneExtensions.GetPhysicsScene(vtm.gameObject.scene);
                Ray ray = svCam.ScreenPointToRay(mp2d + offset * m_drawRadius * 1000f);
                ...
                for (int hit = 0; hit < m_hits.Length; ++hit)
                {
                    if (m_hits[hit].collider != m_meshCollider)
                        continue;

                    int tri = m_hits[hit].triangleIndex;
            ...
            for (int id = 0; id < m_vtxBrush.Count; ++id)            
            {
                int i = m_vtxBrush[id];                
                ...
                //if ( Vector3.Dot( vtm.transform.TransformDirection(m_nrml[i]).normalized, -svCam.transform.forward ) > 0 )
			    {
                    ...
                    if ( dist <= paintRange  )
				    {
					    ...
					    if( !Event.current.alt && Event.current.type == EventType.MouseDrag &&  Event.current.button == 0 /*&& !useHandle*/ )
					    {
                            ...
                            Color targetColor = Event.current.shift ? m_smoothColor : (Event.current.control ? Color.black : Color.white );
            ...
            if (DrawSensorIcon(m_mode == eMode.SENSORS) && !warning)
            {
                ...
                if (!Application.isPlaying && m_mode != eMode.SENSORS)
                {
                    ...
                    for (int i = 0; i < sharedMaterials.Length; ++i)
                    {
                        ...
                        sharedMaterials[i].shader = shaderEditor;
                    }
	    ...
	    void DrawInfo( VertExmotionBase vtm )
	    {
			    ...
			    else if( m_mode == eMode.INFO && fixMaterialID != -1 )
		    {
			    ...
			    if( GUILayout.Button("Fix material") )
			    {
				    ...
				    string shaderName = sharedMaterials[fixMaterialID].shader.name;
                ...
                EditorGUILayout.HelpBox("The name of the shader should begin with 'VertExmotion/'.\nIf you know that your shader uses the VertExmotion function, you can ignore this warning.", MessageType.Info);
    ...
    //tips
	    ...
	    {            
		    ...
		    if( vtm.m_VertExmotionSensors.Count<VertExmotionBase.MAX_SENSOR )
		    {
                ...
                if (GUILayout.Button("Duplicate", EditorStyles.miniButton, GUILayout.Height(14)))
                {
...
#endif
                    {
                        ...
                        {
                            GameObject go = GameObject.Instantiate(vtm.m_VertExmotionSensors[m_sensorId].gameObject, vtm.m_VertExmotionSensors[m_sensorId].gameObject.transform.parent, true);
            ...
            vtm.m_editorParams.m_gradientFactor = EditorGUILayout.Slider(vtm.m_editorParams.m_gradientFactor, 0f, 1f);
            m_matPropBlk.SetFloat("_gradientFactor", vtm.m_editorParams.m_gradientFactor);
            //DrawGradientToggle();
        ...
        */
        ...
        void DrawBrushMenu()
	    {
            ...
            if (vtm.m_mesh.subMeshCount > 1)
            {
                ...
                int selected = EditorGUILayout.IntPopup(m_selectedMaterialSlotID, m_materialSlotsName.ToArray(), m_materialSlotIDs, GUILayout.Width(50));
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                else
                {
                    for (int i = 0; i < sharedMaterials.Length; ++i)
                    {
                        if (sharedMaterials[i] == null)
                            continue;
                        sharedMaterials[i].shader = shaderEdit;
                    }
                }
#endif
            }
            ...
            float min = m_brushMinMax.x;
            float max = m_brushMinMax.y;
            GUILayout.BeginHorizontal();
	    ...
	    {
            float constUnit = HandleUtility.GetHandleSize(svCam.transform.position + svCam.transform.forward) * 10f;
            m_brushSize = m_drawRadius * constUnit;
            ...
            Color c = Event.current.shift ? Color.yellow : (Event.current.control ? Color.red : Color.gray );
		    c.a = .05f + .1f * m_drawIntensity;
		    Handles.color = c;
		    Handles.DrawSolidDisc( mp, -svCam.transform.forward, size);	

		    c = orange;
		    c.a = 1f;
		    Handles.color = c;
		    Handles.DrawWireDisc( mp, -svCam.transform.forward, size);	

		    c.a = .5f;
		    Handles.color = c;
		    Handles.DrawWireDisc( mp, -svCam.transform.forward, size * (1f-m_drawFalloff) );	
	    }
        ...
        void RestoreShaders( VertExmotionBase vtm )
        {
            Material[] sharedMaterials = vtm.renderer.sharedMaterials;
		...
		void PaintAll( bool paintWhite )
		{
            ...
            if (m_selectedMaterialSlotID != 0)
            {
                ...
                {
                    vtm.m_vertexColors[tri[i] + 0] = paintWhite ? Color.white * Mathf.Clamp(m_drawIntensity, m_brushMinMax.x, m_brushMinMax.y) : Color.black;                        
                }                
            }
            else
            {
